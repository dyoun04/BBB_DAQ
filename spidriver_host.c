...
uint8_t pru0_init(void) {
  ...
  retval = prussdrv_map_prumem(PRUSS0_SHARED_DATARAM, (void **) &pru0_sharedram);
  if (retval != 0) {
...
uint8_t pru1_init(void) {
  ...
  // Get pointer to PRU0 dataram.
  retval = prussdrv_map_prumem(PRUSS0_SHARED_DATARAM, (void **) &pru1_sharedram);
  if (retval != 0) {
...
uint8_t spi_writeread_continuous(uint32_t *txdata, int txcnt, uint32_t *rxdata, int rxcnt, int ncnv) {
  ...
  */
  ...
  rxptr = memptr+1;                 // This points to begin of rx data.
...
//---------------------------------------------------------------------------
// These functions break spi_writeread_continuous (above) into
// separate functions, separating the phases to allow for double 
// buffering
//---
// First, a function to start a block transfer from the ADC
//
...
//---
// Second, a function to start a block transfer from the ADC, after 
// waiting for a previous transfer to complete
//
